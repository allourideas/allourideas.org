...
class IdeaMailer < ActionMailer::Base
  ...
  def flag_notification(earl, choice_id, choice_data, explanation, photocracy = false)
    ...
    @bcc = photocracy ? ENV["INFO_PHOTOCRACY_EMAIL"] : ENV["INFO_ALLOURIDEAS_EMAIL"]
    @body[:earl] = earl
    @body[:choice_id] = choice_id
  ...
  def extra_information(user, question_name, information, photocracy = false)
    @recipients = ENV["SIGNUPS_ALLOURIDEAS_EMAIL"]
    @from = ENV["INFO_ALLOURIDEAS_EMAIL"]
  ...
  def setup_email(user, photocracy = false)
    ...
    @from = photocracy ? ENV["INFO_PHOTOCRACY_EMAIL"] : ENV["INFO_ALLOURIDEAS_EMAIL"]
    @subject = photocracy ? "[Photocracy] " : "[All Our Ideas] "
    @sent_on = Time.now
    @body[:user] = user
  ...
  def get_choice_url(earl_name, choice_id, photocracy, option)
    ...
    if photocracy
      choice_url = "http://#{ENV["PHOTOCRACY_HOST"]}#{choice_path}"
    else
      choice_url = "http://#{ENV["HOST"]}#{choice_path}"
    end
    choice_url
  end
end
