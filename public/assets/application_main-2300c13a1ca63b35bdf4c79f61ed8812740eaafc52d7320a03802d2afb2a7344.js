...
AOI.app = (function ($) {
    ...
    $(".collapse").on("show", function (ev) {
      ...
      if (target.is(".map")) {
        var iframe = $(
          "<iframe src='" +
            link.attr("href") +
            "' width='746px' height='370px' frameborder=0 scrolling=no></iframe>"
        );
  ...
  that.createModalFromHref = function (href) {
    var modal = $(
      '<div class="modal"><div class="modal-body"><div class="progress progress-striped active"><div class="bar" style="width: 66%;"></div></div></div><div class="modal-footer"><button class="close" data-dismiss="modal">x</button></div></div>'
    ).modal();
...
AOI.admin = (function ($) {
  ...
  function editQuestion() {
    ...
    if ($("#edit-question").length > 0) {
      $(".modal .new_idea_field").jqEasyCounter({
        target: ".modal .new_idea_counter",
        maxChars: 255,
        maxCharsWarning: 245,
      });
    }
...
jQuery(document).ready(function () {
  ...
  $(".votebox tr.prompt td.idea").each(function (el) {
    $(this).bind("click", function () {
      $([
        $(this).children(".round-filledfg"),
        $(this).children(".round-filled").children(),
      ]).each(function (el) {
  ...
  $(".toggle_choice_status").bind("click", function (ev) {
    ...
    $.post(
      "/questions/" + earl_id + "/choices/" + choice_id + "/toggle.js",
  ...
  $(".toggle_autoactivate_status").bind("click", function (event) {
    ...
    var state = button.attr("data-status");
    $.post(
      "/questions/" + question_id + "/toggle_autoactivate.js",
      { authenticity_token: AUTH_TOKEN },
      function (data) {
        $(".indicator").hide();
        $.unblockUI();
  ...
  $(".view_voter_map").click(function (event) {
    ...
    if (!toggleLinkTextandTargetElement($(this), target_row)) {
      var iframe_html =
        "<tr id=voter_map_row class='row1'><td class='title' colspan='2' height=370px><div class='voter_map_indicator'><img src='/images/indicator.gif' /></div><iframe id='voter_map_iframe' src='" +
        $(this).attr("href") +
        "' width='746px' height='370px' frameborder=0 scrolling=no style='border:1px solid rgb(145,145,145);'></iframe></td></tr>";
